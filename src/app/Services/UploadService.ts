        ...
        this.apiService.UploadFile(formData).subscribe(response => {
            if (Number(response)) {
                ...
                switch (this.getFileType(file)) {
                    case 0:
                        encedMessages = AES.encrypt(`[img]${(<UploadFile>response).path}`, aesKey).toString();
                        break;
                    case 1:
                        encedMessages = AES.encrypt(`[video]${(<UploadFile>response).path}`, aesKey).toString();
                        break;
                    case 2:
                        encedMessages = AES.encrypt(`[file]${(<UploadFile>response).path}`, aesKey).toString();
                        break;
                    default:
                        break;
                }
